...
// --- Last modification: Date 05 April 2007 17:09:02 By Laurent GAY ---

//@BEGIN@
require_once "setup_param.inc.php";
if (is_file("../applis/setup.inc.php"))
	require_once "../applis/setup.inc.php";
else
	...
	$pied_text="	<tr class='pied'>
		<td colspan=2 class='pied'>
			<table border=0 cellspacing=0 cellpadding=0 width='100%'>
				<tr>
					<td class='pied'>$extention_description - Guide d'utilisation";
	if ($includeAll)
	{
		global $num_page;
		$num_page++;
...
function showHelpPage($extension,$helpname,$title='',$includeAll=false)
{
	...
	if (($extension!='') && ($helpname!=''))
	{
		...
		if ($includeAll)
			while (($pos = strpos($content, "<JUMP_PAGE/>"))!==false)
			{
		...
		$HtmlText.="<div class='title'><br><br><br><br><a name='$extension'>$title</a></div>";
        	else
	{
		$HtmlText.="<div class='title'><br><br>$extention_description<br>";
		$HtmlText.="Guide d'utilisation<br>";
		if ($includeAll)
			$HtmlText.="<br><br><br><br>";
		if (is_dir('../applis/images/'))
		{
			if (is_file('../applis/images/image.jpg'))
				$HtmlText.="<img src='../applis/images/image.jpg'/></div>";
			else
				$HtmlText.="<img src='../applis/images/logo.gif'/></div>";
		}
		else
		{
			if (is_file('../extensions/applis/images/image.jpg'))
				$HtmlText.="<img src='../extensions/applis/images/image.jpg'/></div>";
			else
				$HtmlText.="<img src='../extensions/applis/images/logo.gif'/></div>";
		}
	}
	$HtmlText.="</td>
	</tr>";
...
if (array_key_exists('mode',$_GET) && ($_GET['mode']=='manual'))
{
	...
	foreach($MenuList as $menu_item)
	{
		...
		if (count($list_menu)>0)
			foreach($list_menu as $desc)
				echo "\t\t\t<li><table width='100%'><tr><td><a href='#".$extension."_APAS_".$desc[0]."'>".$desc[1]."</a></td><td align='right'>".$chapiter_pages[$extension."@".$desc[0]]."&nbsp;&nbsp;&nbsp;</td></tr></table></li>\n";
		echo "\t\t</ul>\n\t</li>\n";
	}
	echo "</ul>\n";
	echo "</td>
...
{
	if (array_key_exists('extension',$_GET) && array_key_exists('helpname',$_GET))
		echo showHelpPage($_GET['extension'],$_GET['helpname']);
        else
		echo showHelpPage('','');
}
	echo "	</body>
