...
{
	$ext_path="..";
	if (($extension!='CORE') && ($extension!='applis'))
		$ext_path.="/extensions";
	$ext_path.="/$extension";
...
function getHelpTitle($extension,$helpname)
{
	...
	if (!is_null($descripts))
		foreach($descripts as $desc)
			if ($desc[0]==$helpname)
				return $desc[1];
	return "";
}
...
function getHelpContent($extension,$HelpFile,$includeAll=false)
{
        ...
        while (($pos=strpos($line,"href='~"))!==false)
	{
		...
		if (($pos1!==false) && ($pos2!==false))
		{
			...
			if ($includeAll)
				$line=substr($line,0,$pos)."href='#".$ext."_APAS_".substr($line,$pos+$pos2+1);
			else
				$line=substr($line,0,$pos)."href='HelpDefault.inc.php?extension=$ext&helpname=".substr($line,$pos+$pos2+1);
		}
	}
...
function showHelpContent($extension,$helpname,$includeAll=false)
{
    $help_file="..";
    if (($extension!='CORE') && ($extension!='applis'))
        $help_file.="/extensions";
    $help_file.="/$extension/help/$helpname.xhlp";
...
{
	$MenuList=array();
	addItemInList('applis',$MenuList,$includeAll);
	$ext_path="../extensions";
	if ($handle = opendir($ext_path))
    	{
        		while (false !== ($item = readdir($handle)))
            		if (($item != ".") && ($item != "..") && is_dir("$ext_path/$item"))
