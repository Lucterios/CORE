...
class TextFormated 
{
	...
	private function _sub_text($xmlitem) {
		...
		for($idx=0;$idx<=count($xmlitem->getChilds());$idx++) {
			...
			if (isset($xmlitem->getChilds()[$idx])) {
				$this->_sub_text($xmlitem->getChilds()[$idx]);
			}
		}
		$this->B=$last_B;
		$this->I=$last_I;
...
class LucteriosPDF extends FPDF
{
	...
	private function _init() {
		...
		$header=$model->getChildsByTagName('page')[0]->getChildsByTagName('header')[0];
		$this->header_h=(float)$header->getAttributeValue('extent');
		$bottom=$model->getChildsByTagName('page')[0]->getChildsByTagName('bottom')[0];
		$this->bottom_h=(float)$bottom->getAttributeValue('extent');
	...
	private function parse_table($xmltable) {
		...
		foreach($xmlcolumns as $xmlcolumn) {
			...
			$cellcolumns[]=$xmlcolumn->getChildsByTagName('cell')[0];
		}
	...
	public function Header() {
		$header=$this->xml->getResult()->getChildsByTagName('page')[0]->getChildsByTagName('header')[0];
		$this->_parse_comp($header,$this->tMargin);
	}

	public function Footer() {
		$bottom=$this->xml->getResult()->getChildsByTagName('page')[0]->getChildsByTagName('bottom')[0];
		$this->_parse_comp($bottom,-1*$this->bMargin);		
	}
	...
	public function Execute($xml_content) {
		...
		$this->xml->parse();
		$this->_init();
		$bodies=$this->xml->getResult()->getChildsByTagName('page')[0]->getChildsByTagName('body');
