...
abstract class PrintItem {
	...
	public function calculPosition() {
		...
		for($i = 0;$i<$this->x;$i++)
			$this->Left = $this->Left+$this->printAction->m_colWidth[$i];
		for($i = $this->x;$i<($this->x+$this->colspan);	$i++)
			$this->width = $this->width+$this->printAction->m_colWidth[$i];
	}
...
class PrintImage extends PrintItem {
	...
	public function getImgSize() {
		if(($this->ImgSize[0]*$this->DPI)<$this->width) {
			$width = (int)($this->ImgSize[0]*$this->DPI);
			$height = (int)($this->ImgSize[1]*$this->DPI);
		}
		else {
			$width = $this->width;
			$height = (int)($this->width*$this->ImgSize[1]/$this->ImgSize[0]);
		}
...
class PrintTable extends PrintItem {
	...
	public function __construct($XferTable,$printAction) { parent:: __construct($XferTable,$printAction);
		$size_rows = array(0);
		for($i = 0;$i< count($XferTable->m_records);	$i++) {
			$size_rows[$i+1] = 0;
			$this->rows[$i] = array();
		}
		...
		$this->sizeX = 0;
		foreach($this->columns as $column)
			$this->sizeX = $this->sizeX+$column[1];
		$sizeY = 0;
		foreach($size_rows as $size_row)
			$sizeY = $sizeY+$size_row;
		$this->height = 6*$sizeY;
	}
...
class PrintAction {
	...
	private function getPrepPage() {
		...
		if($this->Title != "") {
			...
			$content .= '<header extent="0.0" name="after"/>';
	...
	private function getHeightOfRowspan($PrintItemList,$printItem) {
		...
		$value += $current_height;
		if($value<$printItem->height)
			$value = $printItem->height-$value;
		return $value;
	}
	...
	public function generate() {
		...
		foreach($components_list as $components) {
			$new_item = null;
			if( strtolower( get_class($components)) == 'xfer_comp_image')
				$new_item = new PrintImage($components,$this);
			else if( strtolower( get_class($components)) == 'xfer_comp_grid')
				$new_item = new PrintTable($components,$this);
			else if( strtolower( get_class($components)) == 'xfer_comp_tab')
				$new_item = new PrintTab($components,$this);
			else if( in_array( strtolower( get_class($components)),array('xfer_comp_label','xfer_comp_linklabel','xfer_comp_labelform')))
				$new_item = new PrintLabel($components,$this);
			...
			if($new_item != null) {
				...
				for($i = 0;$i<($new_item->x+$new_item->colspan);$i++) {
					if(! array_key_exists($i,$ColSize))
						$ColSize[$i] = 0;
					if($i >= $new_item->x)
						$ColSize[$i] = max($ColSize[$i],$new_sizeX);
				}
		...
		$total_size = 0;
		for($i = 0;$i<$max_col;$i++)
			$total_size = $total_size+$ColSize[$i];
		$largeur = $this->largeur_page-2*$this->marge_horizontal;
		for($i = 0;$i<$max_col;$i++)
			$this->m_colWidth[$i] = (int)($largeur*$ColSize[$i]/$total_size);
		$this->m_top = 0;
		$last_y = -1;
		$current_height = 0;
		$this->m_last_top = 0;
		foreach($PrintItemList as $item) {
			if($last_y == $item->y) {
				if($item->rowspan == 1)
					...
					$current_height = max($current_height,$this->getHeightOfRowspan($PrintItemList,$item));
			}
			else {
				...
				if($item->rowspan == 1)
					...
					$current_height = $this->getHeightOfRowspan($PrintItemList,$item);
				$last_y = $item->y;
			}
			$item->calculPosition();
