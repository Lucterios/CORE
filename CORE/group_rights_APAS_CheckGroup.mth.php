...
function group_rights_APAS_CheckGroup(&$self,$group,$groupId,$weigth)
{
//@CODE_ACTION@
$return="";
$self->groupref=$group;
...
while ($DBObjextension_rights->fetch())
  if (!in_array($DBObjextension_rights->id,$rights))
    $notrights[$DBObjextension_rights->id]=array($DBObjextension_rights->rightId, $DBObjextension_rights->weigth, $DBObjextension_rights->extensionId);
...
foreach($notrights as $rgh=>$wgt)
{
   ...
   $gr->groupref=$group;
   $gr->groupId=$groupId;
   $gr->extensionId=$wgt[2];
   $gr->rightId=$wgt[0];
   $gr->value=$wgt[1]>$weigth?'o':'n';
   $gr->insert();
}
return $return;
//@CODE_ACTION@
